...
class IncidenciesViewModel(application: Application): AndroidViewModel(application) {
    ...
    private val _adaptador = MutableLiveData<AdaptadorIncidencies>().apply{
        ...
        value= AdaptadorIncidencies(
            ...
            { incidencia: Incidencia, rating: Float -> RatingListenerManager (incidencia, rating)},
    ...
    // Adaptació al RatingBar
    fun RatingListenerManager (incidencia:Incidencia, rating:Float) {
        // Modifiquem la valoració de la incidència
        incidencia.valoracio = rating
        // La guardem amb una corrutina
        viewModelScope.launch(Dispatchers.IO) {
            // Comprovem si s'estava editant o és una nova incidència
            repository.updateIncidencia(incidencia)
        }
    }
    // Fi adaptació RagingBar
